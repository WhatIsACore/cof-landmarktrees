...
map.on('load', () => {
    ...
    fetch('data/landmarkData.json').then(res => res.json()).then(json => landmarkData = json);

    // load species hash information
    fetch('data/speciesHash.json').then(res => res.json()).then(json => speciesHash = json);
});
...
function displayStreetTreeCard(tree) {
    ...
    $('#street-tree-card .card-name').innerHTML = spec[0];
    $('#street-tree-card .card-spec').innerHTML = spec[1];
    $('#street-tree-card .card-addr').innerHTML = tree.properties.A;
...
function displayLandmarkCard(id) {
    ...
    $('#landmark-card .card-photos').innerHTML = `
        <div class='card-photo' style='--photo-src: url(assets/photos/${id}_L.jpg)' data-src='assets/photos/${id}_L.jpg'></div>
        <div class='card-photo captioned' style='--photo-src: url(assets/photos/${id}_S.jpg)' data-src='assets/photos/${id}_S.jpg' data-caption="${caption}"></div>
    `;
    // add click events
